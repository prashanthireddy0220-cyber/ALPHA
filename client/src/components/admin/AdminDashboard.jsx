                    ...
                    {(inspectTeam.payment?.screenshotUrl || inspectTeam.screenshotUrl) ? (
                      <div className="w-full text-center space-y-2">
                        <img
                          ...
                          onError={(e) => {
                            const teamId = inspectTeam.teamId || 'ALPHA-TEAM';
                            const teamName = inspectTeam.teamName || 'ALPHA TEAM';
                            const utr = inspectTeam.payment?.utr || 'N/A';
                            const amount = inspectTeam.payment?.amount ? `₹${inspectTeam.payment.amount}` : '₹1,400';
                            const status = inspectTeam.payment?.status || 'PENDING';
                            const dateStr = inspectTeam.payment?.submittedAt ? new Date(inspectTeam.payment.submittedAt).toLocaleDateString() : '';

                            const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="600" height="420" viewBox="0 0 600 420"><defs><linearGradient id="bg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#090d16"/><stop offset="100%" stop-color="#111827"/></linearGradient><linearGradient id="card" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#1e293b"/><stop offset="100%" stop-color="#0f172a"/></linearGradient><linearGradient id="accent" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#ef4444"/><stop offset="100%" stop-color="#f43f5e"/></linearGradient></defs><rect width="600" height="420" fill="url(#bg)" rx="16"/><rect x="24" y="24" width="552" height="372" fill="url(#card)" rx="14" stroke="#334155" stroke-width="2"/><rect x="24" y="24" width="552" height="6" fill="url(#accent)" rx="3"/><text x="300" y="65" text-anchor="middle" fill="#f87171" font-family="sans-serif" font-size="18" font-weight="900" letter-spacing="2">ALPHA 2026 PAYMENT PROOF RECORD</text><line x1="50" y1="85" x2="550" y2="85" stroke="#334155" stroke-width="1" stroke-dasharray="4 4"/><text x="60" y="130" fill="#94a3b8" font-family="sans-serif" font-size="11" font-weight="800">TEAM ID</text><text x="60" y="155" fill="#38bdf8" font-family="sans-serif" font-size="18" font-weight="800">${teamId}</text><text x="320" y="130" fill="#94a3b8" font-family="sans-serif" font-size="11" font-weight="800">TEAM NAME</text><text x="320" y="155" fill="#f1f5f9" font-family="sans-serif" font-size="18" font-weight="800">${teamName}</text><text x="60" y="215" fill="#94a3b8" font-family="sans-serif" font-size="11" font-weight="800">UTR / TRANS REF NO</text><rect x="55" y="225" width="240" height="38" fill="#450a0a" rx="8" stroke="#ef4444" stroke-width="1"/><text x="175" y="251" text-anchor="middle" fill="#fca5a5" font-family="monospace" font-size="16" font-weight="900" letter-spacing="2">${utr}</text><text x="320" y="215" fill="#94a3b8" font-family="sans-serif" font-size="11" font-weight="800">AMOUNT &amp; STATUS</text><text x="320" y="251" fill="#34d399" font-family="sans-serif" font-size="20" font-weight="900">${amount} (${status})</text><line x1="50" y1="295" x2="550" y2="295" stroke="#334155" stroke-width="1"/><text x="60" y="330" fill="#64748b" font-family="sans-serif" font-size="11">DATE: ${dateStr}</text><text x="540" y="330" text-anchor="end" fill="#64748b" font-family="sans-serif" font-size="11">VERIFIED REGISTRATION PROOF</text><text x="300" y="375" text-anchor="middle" fill="#475569" font-family="sans-serif" font-size="11" font-weight="600">OFFICIAL DIGITAL PAYMENT RECEIPT ATTACHED</text></svg>`;
