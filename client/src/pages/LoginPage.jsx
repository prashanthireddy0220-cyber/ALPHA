        ...
        <TiltCard className="p-8 md:p-10 rounded-3xl glass-card border border-sky-500/30 shadow-[0_0_50px_rgba(0,240,255,0.2)] bg-slate-950/80 backdrop-blur-2xl text-center">
          ...
          {user ? (
            <div className="space-y-4 mb-4">
              <div className="p-4 rounded-2xl bg-sky-950/50 border border-sky-500/30 text-left">
                <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-1">CURRENTLY SIGNED IN AS</span>
                <span className="text-xs font-bold text-white block truncate">{user.name || 'Student'}</span>
                <span className="text-[11px] text-cyan-300 font-mono block truncate">{user.email}</span>
              </div>

              <button
                type="button"
                onClick={() => navigate(user.teamId ? '/dashboard' : '/register')}
                className="w-full py-3.5 px-4 text-xs font-extrabold tracking-wider text-black bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-500 hover:from-sky-300 hover:to-cyan-400 rounded-2xl shadow-[0_0_25px_rgba(0,240,255,0.4)] transition-all cursor-pointer uppercase"
              >
                {user.teamId ? 'CONTINUE TO TEAM DASHBOARD' : 'PROCEED TO REGISTRATION'}
              </button>
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-3 px-4 text-xs font-bold tracking-wider text-slate-300 hover:text-white glass-button rounded-2xl transition-all flex items-center justify-center gap-2"
              >
